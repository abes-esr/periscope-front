<template>
   <v-expansion-panel class="outlined-app" style="padding: 0.5em 0.5em 0.5em 0.5em">
      <v-row :align="getVerticalAlignValue(1)">
         ...
         <v-col xs="12" sm="8" lg="10">
            <v-expansion-panel-header>
               <template v-slot:default="{open}">
                  <v-row no-gutters>
                     <v-col xs="12" sm="4" lg="2"> Recherche par PPN </v-col>
                     <v-col xs="12" sm="8" lg="10" class="text--secondary">
                        <v-fade-transition leave-absolute>
                           <span v-if="open || blocPpn.ppnListString.length === 0" key="0"> Saisissez des n° de PPN </span>
                           <span v-else key="1"> {{ blocPpn.ppnListString }} </span>
                        </v-fade-transition>
                     </v-col>
                  </v-row>
               </template>
            ...
            <v-expansion-panel-content>
               <v-row :align="getVerticalAlignValue(1)" :justify="getHorizontalJustifyValue(1)" no-gutters>
                  <v-col sm="8" lg="10">
                     ...
                     <span>{{ this.blocPpn.ppnHandler }}</span>
                     <!--Internal Operator-->
                  </v-col>
                  <v-col sm="4" lg="2" style="padding-left: 0.5em; padding-top: 0.5em">
